#!/usr/bin/env python3
"""
IoT Streaming Pipeline — RETRY TEST variant — EMR on EKS (Bronze + Silver)
Writes to *_retrytest tables in bs-iot-tables-poc S3 Tables bucket.

This variant is identical to the 6exec script with ONE addition:
fail_if_marker_present() is called at the start of _run(). It checks S3 for
a marker file. If present, it deletes the marker and raises RuntimeError —
intentionally failing the driver to validate retryPolicyConfiguration.
On the retry attempt, the marker is gone and the job runs normally.
"""
...
CONSUMER_GROUP      = "emr_eks_streaming_poc_retrytest"

BASE                = f"s3://{S3_BUCKET}"
CHECKPOINT_COMBINED = f"{BASE}/checkpoints/eks/combined/iot_v8_retrytest"
...
BRONZE_TABLE        = f"`{CATALOG}`.bronze.iot_retrytest"
SILVER_LATEST_TABLE = f"`{CATALOG}`.silver.iot_events_latest_retrytest"
SILVER_VALID_TABLE  = f"`{CATALOG}`.silver.iot_events_latest_valid_retrytest"

REGION              = "ap-south-1"
CW_NAMESPACE        = "BatterySmart/IoTStreaming/RetryTest"
...
# ─── RETRY TEST: marker-driven intentional failure ────────────────────────
# On startup, if the S3 marker exists, delete it and raise. The driver exits
# non-zero → EMR retryPolicyConfiguration fires → new driver pod is created
# → marker is gone → normal execution. Validates that retry actually works.
RETRY_TEST_MARKER_BUCKET = "emr-migration-poc"
RETRY_TEST_MARKER_KEY    = "test/retry-fail-marker"
EVICT_TEST_MARKER_KEY    = "test/evict-sigterm-marker"

def fail_if_marker_present() -> None:
    s3_client = boto3.client("s3", region_name=REGION)
    try:
        s3_client.head_object(Bucket=RETRY_TEST_MARKER_BUCKET, Key=RETRY_TEST_MARKER_KEY)
    except s3_client.exceptions.ClientError as e:
        code = e.response.get("Error", {}).get("Code")
        if code in ("404", "NoSuchKey", "NotFound"):
            logger.info("[RETRY TEST] Marker absent — proceeding normally.")
            return
        raise
    # Marker is present — delete and fail this attempt intentionally.
    logger.warning("[RETRY TEST] Marker present at "
                   f"s3://{RETRY_TEST_MARKER_BUCKET}/{RETRY_TEST_MARKER_KEY} — "
                   "deleting marker and raising to test retry policy.")
    s3_client.delete_object(Bucket=RETRY_TEST_MARKER_BUCKET, Key=RETRY_TEST_MARKER_KEY)
    raise RuntimeError(
        "INTENTIONAL_RETRY_TEST_ERROR: S3 marker was present at "
        f"s3://{RETRY_TEST_MARKER_BUCKET}/{RETRY_TEST_MARKER_KEY}. "
        "Failing this attempt to validate retryPolicyConfiguration.maxAttempts. "
        "Marker has been deleted; the next driver attempt should succeed."
    )


def maybe_schedule_self_sigterm() -> None:
    """If the eviction marker is present, delete it and schedule SIGTERM to PID 1
    after 60s. Simulates kubelet eviction (e.g., Karpenter Drift). Container
    exits non-zero → EMR retryPolicyConfiguration fires.

    Belt-and-suspenders: if PID 1 doesn't terminate within 60s of SIGTERM,
    force os._exit(143) so the test still produces a non-zero exit and triggers
    retry. Without this, an excessively-graceful JVM exit-0 would NOT trigger
    retry, and we'd burn the test silently.
    """
    s3 = boto3.client("s3", region_name=REGION)
    try:
        s3.head_object(Bucket=RETRY_TEST_MARKER_BUCKET, Key=EVICT_TEST_MARKER_KEY)
    except s3.exceptions.ClientError as e:
        code = e.response.get("Error", {}).get("Code")
        if code in ("404", "NoSuchKey", "NotFound"):
            logger.info("[EVICT TEST] Marker absent — no self-SIGTERM scheduled.")
            return
        raise
    logger.warning(f"[EVICT TEST] Marker present at "
                   f"s3://{RETRY_TEST_MARKER_BUCKET}/{EVICT_TEST_MARKER_KEY} — "
                   "deleting marker and scheduling SIGTERM to PID 1 in 60s.")
    s3.delete_object(Bucket=RETRY_TEST_MARKER_BUCKET, Key=EVICT_TEST_MARKER_KEY)

    def _delayed_sigterm() -> None:
        time.sleep(60)
        logger.warning("[EVICT TEST] Sending SIGTERM to PID 1 now (simulating kubelet eviction).")
        try:
            os.kill(1, signal.SIGTERM)
        except Exception as e:
            logger.error(f"[EVICT TEST] os.kill(1, SIGTERM) failed: {e}")
        time.sleep(60)
        logger.warning("[EVICT TEST] Still alive 60s after SIGTERM — forcing os._exit(143) to guarantee non-zero exit.")
        os._exit(143)

    threading.Thread(target=_delayed_sigterm, daemon=True).start()
...
def _run() -> None:
    # RETRY TEST: fail fast if marker present, before SparkSession init.
    fail_if_marker_present()

    spark = (SparkSession.builder
        .appName("iot-streaming-bronze-silver-eks-retrytest")
    ...
    def write_silver(prepared_df: DataFrame, batch_id: int) -> None:
        ...
        try:
            ...
            # ── silver.iot_events_latest_valid_retrytest ──────────────────────
            ...
            # ── silver.iot_events_latest_retrytest ────────────────────────────
    ...
    # EVICT TEST: schedule self-SIGTERM AFTER query starts, so a batch can run first.
    maybe_schedule_self_sigterm()
